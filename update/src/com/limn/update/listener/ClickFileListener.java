...
public class ClickFileListener implements OnClickListener {
	...
	public void onClick(View v) {
		...
		if (name.equals("安装")) {
			...
			String filePath = StaticHttp.APK_FILES_PATH + "/" + fileDownLoadPath;
		...
		if (!isLoading && name.equals("下载")) {
			isLoading = true;
			new Thread(new DownLoadFile(fileDownLoadPath, ((Button) v))).start();
	...
	private void openFile(String fileDownLoadPath) {
		...
		intentInstall.setDataAndType(Uri.fromFile(new File(fileDownLoadPath)), "application/vnd.android.package-archive");
	...
	class DownLoadFile implements Runnable {
		 ...
		 * @throws IOException
		 */
		public void down_file(String path) throws IOException {

			String url = path;
			
			url = StaticHttp.DOWNLOAD_IP + encode(url, "UTF-8");

			// String fileNameTextView.setText(fileName);
			// 获取文件名
